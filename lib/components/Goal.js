...
export default class Goal extends Component {
  ...
  get reference() {
    ...
    return firebase
      .database()
      .ref(`${currentUser}/${this.props.dataId}`);
  }
  ...
  componentDidMount() {
    this
      ...
      .on('value', (snapshot) => {
        ...
        actions = splitObject(actions).map(action => Object.assign({
          key: action.key
        }, action.value));
  ...
  render() {
    ...
    return (
      <section accessKey={dataId} className="goalContainer">
        <article className="Goal">
          <h3 className="goalTitle">{this.props.title}</h3>
          <div className="addNewAction"></div>
        </article>
        <article className="ActionRoom">
          <CreateAction reference={this.reference} accessKey={dataId}/>
          <button className="sortButton" onClick={() => this.sortActions()}>Sort</button>
